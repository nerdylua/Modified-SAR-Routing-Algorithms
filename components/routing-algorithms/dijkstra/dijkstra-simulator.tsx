...
import { runDijkstra, DijkstraStep } from './logic/dijkstra';
...
// NEW: Full Network Analysis Comparison Data Interface
...
const initialNodes: SimulationNode[] = [
  ...
  // Upper path (secure but longer)
  { id: 'B', type: 'networkNode', data: { label: 'B (Secure)', type: 'switch', distance: '∞', visited: false, isCurrent: false }, position: { x: 200, y: 100 } },
  { id: 'C', type: 'networkNode', data: { label: 'C (Secure)', type: 'switch', distance: '∞', visited: false, isCurrent: false }, position: { x: 350, y: 80 } },
  { id: 'D', type: 'networkNode', data: { label: 'D (Secure)', type: 'router', distance: '∞', visited: false, isCurrent: false }, position: { x: 500, y: 100 } },
  
  // Lower path (risky but shorter)
  { id: 'E', type: 'networkNode', data: { label: 'E (Risky)', type: 'switch', distance: '∞', visited: false, isCurrent: false }, position: { x: 350, y: 300 } },
...
const initialEdges: SimulationEdge[] = [
  // Upper secure path (longer distances, low security risk)
  { id: 'A-B', source: 'A', target: 'B', type: 'networkEdge', label: '4', data: { weight: 4, isHighlighted: false, securityRisk: 0.1 }, markerEnd: { type: MarkerType.ArrowClosed } },
  { id: 'B-C', source: 'B', target: 'C', type: 'networkEdge', label: '3', data: { weight: 3, isHighlighted: false, securityRisk: 0.1 }, markerEnd: { type: MarkerType.ArrowClosed } },
  { id: 'C-D', source: 'C', target: 'D', type: 'networkEdge', label: '3', data: { weight: 3, isHighlighted: false, securityRisk: 0.2 }, markerEnd: { type: MarkerType.ArrowClosed } },
  { id: 'D-F', source: 'D', target: 'F', type: 'networkEdge', label: '4', data: { weight: 4, isHighlighted: false, securityRisk: 0.1 }, markerEnd: { type: MarkerType.ArrowClosed } },
  
  // Lower risky path (shorter distances, high security risk)
  { id: 'A-E', source: 'A', target: 'E', type: 'networkEdge', label: '2', data: { weight: 2, isHighlighted: false, securityRisk: 0.8 }, markerEnd: { type: MarkerType.ArrowClosed } },
  { id: 'E-F', source: 'E', target: 'F', type: 'networkEdge', label: '2', data: { weight: 2, isHighlighted: false, securityRisk: 0.9 }, markerEnd: { type: MarkerType.ArrowClosed } },
  
  // Middle alternative path (balanced)
  { id: 'A-G', source: 'A', target: 'G', type: 'networkEdge', label: '3', data: { weight: 3, isHighlighted: false, securityRisk: 0.3 }, markerEnd: { type: MarkerType.ArrowClosed } },
  { id: 'G-F', source: 'G', target: 'F', type: 'networkEdge', label: '3', data: { weight: 3, isHighlighted: false, securityRisk: 0.4 }, markerEnd: { type: MarkerType.ArrowClosed } },
  
  // Cross connections for more routing options
  { id: 'B-G', source: 'B', target: 'G', type: 'networkEdge', label: '2', data: { weight: 2, isHighlighted: false, securityRisk: 0.2 }, markerEnd: { type: MarkerType.ArrowClosed } },
  { id: 'G-D', source: 'G', target: 'D', type: 'networkEdge', label: '2', data: { weight: 2, isHighlighted: false, securityRisk: 0.3 }, markerEnd: { type: MarkerType.ArrowClosed } },
  { id: 'C-E', source: 'C', target: 'E', type: 'networkEdge', label: '2', data: { weight: 2, isHighlighted: false, securityRisk: 0.6 }, markerEnd: { type: MarkerType.ArrowClosed } },
  { id: 'G-E', source: 'G', target: 'E', type: 'networkEdge', label: '1', data: { weight: 1, isHighlighted: false, securityRisk: 0.5 }, markerEnd: { type: MarkerType.ArrowClosed } },
];

let idCounter = 2;
const getNodeId = (type: 'router' | 'switch') => `${type.charAt(0)}${++idCounter}`;

const initialSimulationState: SimulationState<DijkstraStep> = {
  nodes: initialNodes,
  edges: initialEdges,
  steps: [],
  currentStepIndex: -1,
...
function DijkstraSimulatorFlow() {
  const [nodes, setNodes, onNodesChangeInternal] = useNodesState(initialNodes);
  const [edges, setEdges, onEdgesChangeInternal] = useEdgesState(initialEdges);
  const [simulationState, setSimulationState] = useState<SimulationState<DijkstraStep>>(initialSimulationState);
  ...
  // NEW: Updated comparison state for full network analysis
  ...
  const calculateRoutingMetrics = useCallback((
    ...
    const distanceCost = alpha * totalDistance;
    const securityCost = beta * totalSecurityRisk;
    const totalCost = distanceCost + securityCost;
  ...
  const applyStep = useCallback((step: DijkstraStep) => {
    setNodes((nds) =>
      nds.map((node) => {
        ...
        const visited = step.visited.has(node.id);
        return {
          ...node,
          data: {
            ...nodeData,
          ...
          className: isCurrent ? 'node-current' : visited ? 'node-visited' : '',
        };
      })
    );
    setEdges((eds) =>
  ...
  const handleSetStartNode = useCallback((nodeId: string | null) => {
    ...
    if (nodes.length > 0) {
        ...
        if (steps.length > 0) {
            ...
            toast.success(`Dijkstra initialized. Start node: ${nodeId}. Click play to begin.`);
        } else {
  ...
  // NEW: Full Network Analysis Function
  const runFullNetworkAnalysis = useCallback((startNodeId: string) => {
    ...
    // Run Classic Dijkstra (β=0)
    const classicSteps = runDijkstra(currentNodes, currentEdges, startNodeId, {
      mode: 'classic',
      beta: 0
    });
    
    // Run SAR Dijkstra (user-selected β)
    const sarSteps = runDijkstra(currentNodes, currentEdges, startNodeId, {
      mode: 'sar',
      beta: securityFactor
    });
      ...
      <div className="w-full h-full flex flex-col border rounded-lg overflow-hidden bg-card">
        ...
        <div className="p-3 border-b bg-muted/20">
          <div className="flex items-center justify-center gap-4">
            ...
            {/* NEW: Full Network Analysis Button */}
            <Button 
              size="sm" 
              variant="default" 
              ...
              <div className="flex-1 overflow-y-auto">
                {simulationState.steps.length > 0 && simulationState.currentStepIndex >= 0 ? (
                  <div className="space-y-1">
                    ...
                    {/* Show current distances as priority queue representation */}
                    {Object.entries(simulationState.steps[simulationState.currentStepIndex]?.distances || {})
                      .filter(([nodeId, distance]) => 
                        distance !== Infinity && 
                        !simulationState.steps[simulationState.currentStepIndex]?.visited.has(nodeId)
                      )
                      ...
                      .map(([nodeId, distance], idx) => (
                        <div
                          ...
                          className={`text-xs py-2 px-3 rounded-lg border transition-all duration-200 ${
                            idx === 0 
                              ? 'bg-gradient-to-r from-green-100 to-green-200 text-green-900 font-bold border-green-400 shadow-sm' 
                        ...
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <span className={`font-mono ${idx === 0 ? 'text-green-800' : 'text-gray-600'}`}>
                  ...
                  <div className="text-xs text-gray-500 italic py-3 text-center">
                    Priority queue will appear during simulation...
                  </div>
                )}
              </div>
            </div>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {Object.entries(simulationState.steps[simulationState.currentStepIndex]?.distances || {})
                        ...
                        .map(([nodeId, distance]) => {
                          ...
                          const isVisited = simulationState.steps[simulationState.currentStepIndex]?.visited.has(nodeId);
                          
                          return (
                            <tr key={nodeId} className={`hover:bg-gray-50 ${isVisited ? 'bg-purple-50' : ''}`}>
        ...
        {/* NEW: Full Network Comparison Results Section */}
        {comparisonData.isComparisonMode && Object.keys(comparisonData.classicResults).length > 0 && (
          <div className="p-4 border-t bg-gradient-to-r from-blue-50 to-green-50">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-6 gap-4 mb-6">
              ...
              {/* NEW: Security Improvement Metric */}
                ...
                <div className="text-sm text-purple-700">
                  {(() => {
                    ...
                    if (riskReduction > 10) {
                      ...
                      return `⚠️ SAR shows minimal security benefit in this topology. Classic Dijkstra may be sufficient for performance-focused scenarios.`;
                    }
                  })()}
                </div>
              </div>
                  ...
                  <div className="mt-2 text-xs text-gray-600">
                    ...
                    {algorithmMode === 'classic' && (
                      <span className="block mt-1 text-blue-600">
                        Note: Classic Dijkstra ignores security risk (β=0)
                      </span>
                    )}
                  </div>
                </div>
